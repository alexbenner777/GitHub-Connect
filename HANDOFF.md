...
**Продакшн (Railway):** `https://trends-landing-production.up.railway.app`  
...
```typescript
app.use(cors({
  origin: ["https://trends-landing-production.up.railway.app", process.env.REPLIT_DEV_DOMAIN ? `https://${process.env.REPLIT_DEV_DOMAIN}` : ""],
  credentials: true
}));
```
...
Настроен в `railway.toml`. Каждый `git push` в ветку `main` автоматически пересобирает и деплоит проект.
